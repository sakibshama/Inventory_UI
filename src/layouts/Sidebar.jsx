    ...
    return (
        <aside id="layout-menu" className="layout-menu menu-vertical menu bg-menu-theme">
            <div className="app-brand demo">
                <Link aria-label='Navigate to homepage' to="/" className="app-brand-link lgo">
                <img className='global-logo' src={logo} alt="" />
                    <span className='logo-text'>VIP Global Ltd.</span>
                </Link>
        ...
        <li className={`menu-item ${isActive || isSubmenuActive ? 'active' : ''} ${hasSubmenu && isSubmenuActive ? 'open' : ''}`}>
            ...
            >
                ...
                <div>{item.text}</div> {item.available === false && (
                    <div className="badge bg-black bg-label-bg-primary fs-tiny rounded-pill ms-auto">Pro</div>
                )}
            </NavLink>
            {item.submenu && (
